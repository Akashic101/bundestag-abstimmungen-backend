...
app.get("/mitglieder/", (req, res) => {
  db.all(
    "SELECT DISTINCT Name, Vorname, FraktionGruppe FROM ExcelData",
    (err, rows) => {
      if (err) {
        console.error(err.message);
        return res.status(500).json({ error: "Internal Server Error" });
...
app.get("/mitglieder/:mitglied", (req, res) => {
  ...
  db.all(
    "SELECT * FROM ExcelData WHERE Bezeichnung = ?",
    [mitglied],
    (err, rows) => {
      if (err) {
        console.error(err.message);
...
app.get("/fraktiongruppe/:fraktiongruppe/mitglieder", (req, res) => {
  ...
  db.all(
    "SELECT DISTINCT Name, Vorname, Bezeichnung FROM ExcelData WHERE FraktionGruppe = ?",
    [fraktiongruppe],
    (err, rows) => {
      if (err) {
        console.error(err.message);
